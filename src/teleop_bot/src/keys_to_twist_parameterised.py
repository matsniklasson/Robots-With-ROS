...
def keys_cb(msg: String, twist_pub):
...
if __name__ == '__main__':
    ...
    g_last_twist = Twist() # initalise to zero
    
    if rospy.has_param('~linear_scale'):
        g_vel_scales[1] = rospy.get_param('~linear_scale')
    else:
        rospy.logwarn("linear scale not provided; using %s" % g_vel_scales[1])

    if rospy.has_param('~angular_scale'):
        g_vel_scales[0] = rospy.get_param('~angular_scale')
    else:
        rospy.logwarn("angular scale not provided; using %s" % g_vel_scales[0])
